...
# 1 - read input data ---------------------------------------------------------
## projects data ----
projects_data <- read_excel(path = here('01_projects_repository', 
...
## awards data ----
awards_data <- read_excel(path = here('01_projects_repository', 
                                      'project_repository_tables.xlsm'), 
                          sheet = 'awards_table',
                          col_types = 'text') |> 
    clean_names()
### remove apostrophe from award names (seems to cause problems with the categories on the site) ----
awards_data <- awards_data |> 
    mutate(award_name = str_replace_all(string = award_name, 
                                        pattern = "'", 
                                        replacement = ""))

## topics data ----
topics_data <- read_excel(path = here('01_projects_repository', 
                                      'project_repository_tables.xlsm'), 
                          sheet = 'topics_table',
                          col_types = 'text') |> 
    clean_names()
...
## events data ----
events_data <- read_excel(path = here('01_projects_repository', 
                                      'project_repository_tables.xlsm'), 
                          sheet = 'events_table',
                          col_types = 'text') |> 
    clean_names()
...
## markdown templates ----
projects_template <- read_file(here('02_render_markdown_files', 
                                    'project_template.txt'))

awards_template <- read_file(here('02_render_markdown_files', 
                                  'awards_template.txt'))

topics_template <- read_file(here('02_render_markdown_files', 
                                  'topics_template.txt'))

events_template <- read_file(here('02_render_markdown_files', 
                                  'events_template.txt'))
...
## add badge to awards ----
projects_data <- projects_data |> 
    mutate(awards_full_names = 
               reduce2(.x = awards_data$award_name, 
                       .y = paste0('"', awards_data$award_name, ' ', awards_data$award_badge, '"'), 
...
pwalk(
    .l = list(projects_data$project_id, # ..1
...
awards_data <- awards_data |> 
    ...
    mutate(awards_badge_render = 
               str_replace_all(string = award_name, 
                               pattern = award_name, 
...
awards_text <- paste0(awards_template, 
                      pmap_chr(.l = list(awards_data$award_name, # ..1
                                         awards_data$award_badge, # ..2
                                         awards_data$award_description, # ..3
                                         awards_data$awards_badge_render, # ..4
                                         awards_data$awards_full_names), # ..5
                               .f = ~ paste0('## ', ..1, ' ', ..4,
                                             '\r\n\r\n', 
                                             ..3, '\r\n\r\n',
                                             '[', ..1, 
...
## create text for markdown file ----
topics_text <- paste0(topics_template, 
                      pmap_chr(.l = list(topics_data$topic_name, # ..1
                                         topics_data$topic_description), # ..2
                               .f = ~ paste0('## ', ..1,
                                             '\r\n\r\n', 
                                             ..2, '\r\n\r\n',
                                             '[', ..1, 
...
## create text for markdown file ----
events_text <- paste0(events_template, 
                      pmap_chr(.l = list(events_data$event_name, # ..1
                                         events_data$event_description), # ..2
                               .f = ~ paste0('## ', ..1,
                                             '\r\n\r\n', 
                                             ..2, '\r\n\r\n',
                                             '- [', ..1,
